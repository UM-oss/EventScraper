...
def _add_event(db, **kw):
    defaults = dict(
        title="Default", date_start=date(2026, 6, 1), source_id="x", dedup_hash="x",
    )
    defaults.update(kw)
    e = Event(**defaults)
    db.add(e)
...
def test_dedup_exact_normalized_title(db_session):
    _add_event(db_session, title="Koncert Siddharte", date_start=date(2026, 6, 1))
    res = check_dedup(db_session, "Koncert Siddharte", date(2026, 6, 1))
...
def test_dedup_different_time_strict_threshold(db_session):
    ...
    _add_event(db_session, title="Wigmorski solisti", date_start=date(2026, 6, 1))
    # 100% match
    res = check_dedup(db_session,
...
def test_dedup_different_time_low_similarity_returns_new(db_session):
    _add_event(db_session, title="Koncert Siddharte", date_start=date(2026, 6, 1))
    res = check_dedup(db_session, "Predavanje o astronomiji", date(2026, 6, 1))
    assert res.decision == "new"
...
def test_dedup_threshold_can_be_lowered(db_session):
    _add_event(db_session,
               title="Festival glasbe", date_start=date(2026, 6, 1),
               time_start="20:00")
...
def test_dedup_result_includes_matched_event_id(db_session):
    e = _add_event(db_session, title="Match me", date_start=date(2026, 6, 1))
    res = check_dedup(db_session, "Match me", date(2026, 6, 1))
...
def test_backward_compat_is_duplicate_fuzzy(db_session):
    _add_event(db_session, title="Original event", date_start=date(2026, 6, 1))
    assert is_duplicate_fuzzy(db_session, "Original event", date(2026, 6, 1)) is True
    assert is_duplicate_fuzzy(db_session, "Nepovezan dogodek", date(2026, 6, 1)) is False
...
def test_compute_dedup_hash_deterministic():
    h1 = compute_dedup_hash("Koncert", date(2026, 6, 1))
    h2 = compute_dedup_hash("KONCERT", date(2026, 6, 1))  # case-insensitive
    h3 = compute_dedup_hash("Koncert", date(2026, 6, 2))
    assert h1 == h2
    assert h1 != h3
